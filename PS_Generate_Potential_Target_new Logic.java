...
webservice static Integer createPotentialTargets(){
    ...
    List<UniversityCourse__c> CoursePTCheck = new List<UniversityCourse__c>();
    Set<Id> setApptusClassificationId =  new Set<Id>();
    Integer Returnvalue = 0;
    List<UniversityCourse__c> CourseCount = New List<UniversityCourse__c>();
    ...
    if(!usrTerritoryLSt.isEmpty()){
        ...
        if(loggedUserObj.Price_List__c!=null){
            ...
            for(UniversityCourse__c courseObj : [Select id,Account__c,Course_Name__c,PT_Query_Check__c,
                  (Select id,Course__c,Pearson_Course_Code__c,Pearson_Course_Code__r.name,Pearson_Course_Code__r.Pearson_Course_Structure_Code__c from Pearson_Course_Equivalent__r where 
                  Pearson_Course_Code__c in:setApptusClassificationId and Course__r.Account__c in:linkedAccSet and primary__c = true order by lastmodifieddate desc limit 1),
                  ...
                  from UniversityCourse__c where id in (Select course__c from Pearson_Course_Equivalent__c where Pearson_Course_Code__c in:setApptusClassificationId and Course__r.Account__r.id in:linkedAccSet) and PT_Query_Check__c = false limit 1]){
                System.debug('courseObj -->'+courseObj);
                CoursePTCheck.add(courseObj);
         ...
         for(UniversityCourse__c  courseObj:mapCourse.values()){
             if(mapCourseAsset.containsKey(courseObj.id)){
                 ...
                 if(mapCourseEquiv.containsKey(courseObj.id))
                     ...
                     //Modified by Rahul for the incident INC1995375 (Enhancement Request) starts
                     // targetObj.PCC_c__c = mapCourseEquiv.get(courseObj.id).Pearson_Course_Code__r.Pearson_Course_Structure_Code__c+' '+courseObj.Course_Name__c;
                      //Modified by Rahul for the incident INC1995375 (Enhancement Request) ends
            ...
            if(!lstPotentialTarget.isEmpty()){
                 List<UniversityCourse__c> updatecourse = new List<UniversityCourse__c>();
                 Database.SaveResult[] lstResult= Database.insert(lstPotentialTarget, false);
                 for(UniversityCourse__c UpdtePTCorse : CoursePTCheck){
                       UpdtePTCorse.PT_Query_Check__c = true; 
                       updatecourse.add(UpdtePTCorse);
                    }
                 Update updatecourse;
                 CourseCount = [Select id,Account__c,Course_Name__c,PT_Query_Check__c,
                  (Select id,Course__c,Pearson_Course_Code__c,Pearson_Course_Code__r.name,Pearson_Course_Code__r.Pearson_Course_Structure_Code__c from Pearson_Course_Equivalent__r where 
                  Pearson_Course_Code__c in:setApptusClassificationId and Course__r.Account__c in:linkedAccSet and primary__c = true order by lastmodifieddate desc limit 1),
                  (select id,UniversityCourse__c,Opportunity__r.Selling_Period__c from OpportunityUniversityCourses__r where 
                  Opportunity__r.Selling_Period__c=:sellingSeason),
                  ...
                  Product2.Next_Edition__r.Relevance_Value__c,Product2.Copyright_Year__c from Products_In_Use__r  
                  order by primary__c desc,Product2.Copyright_Year__c asc limit 1)
                  from UniversityCourse__c where id in (Select course__c from Pearson_Course_Equivalent__c where Pearson_Course_Code__c in:setApptusClassificationId and Course__r.Account__r.id in:linkedAccSet) and PT_Query_Check__c = false];
                  System.debug('CourseCount --->'+CourseCount.size());
          ...
          if(CourseCount.size() != null){
              Returnvalue = CourseCount.size();
            }
            return Returnvalue ;
    }
}